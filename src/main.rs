...
async fn run_event_loop(
    ...
    loop {
        ...
        } else if app.visualizer_mode.is_some() {
        ...
        if crossterm::event::poll(timeout)? {
            match event::read()? {
                ...
                // Resizing (e.g. focus/DPI changes, the terminal getting
                // shrunk and grown back) can leave stale glyphs on screen
                // that ratatui's diff-based redraw doesn't always catch —
                // some terminal emulators don't fully erase the old
                // viewport on resize. `clear()` forces a full repaint on
                // the very next `terminal.draw()` instead of leaving it to
                // chance.
                Event::Resize(_, _) => {
                    terminal.clear()?;
                }
