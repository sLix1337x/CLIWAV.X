...
pub struct App {
    ...
    pub visualizer_mode: Option<VisualizerMode>,
    ...
    /// Digital-rain + rising-particle animation state (see `ui::visualizer`).
    /// Interior-mutable for the same reason `artwork_cache` is: rendering
    /// needs to advance a simulation frame by frame, but every `ui::*::draw`
    /// function only gets `&App`.
    pub visualizer_fx: std::cell::RefCell<VisualizerFx>,
    ...
    pub picker: Picker,
    /// Whether the terminal background is dark, detected once at startup —
    /// the UI's filled surfaces build their base color from this so they
    /// don't render as hard black blocks on a light-themed terminal.
    pub is_dark_bg: bool,
...
/// `from_fontsize` regardless. Plain conhost (cmd.exe / PowerShell in the
/// legacy console) never answers, so querying there just pays a full 1s
/// stall for the same halfblocks fallback it would've used anyway — skip
/// straight to the fallback for those. But WezTerm and current Windows
/// Terminal builds (`WT_SESSION` set) *do* answer and support real graphics
/// protocols (Sixel/Kitty) — querying them is what stops every Windows user
/// from being force-downgraded to blocky halfblocks art regardless of what
/// terminal they're actually running. Non-Windows terminals always query.
fn detect_picker() -> Picker {
    #[cfg(windows)]
    {
        let graphical_terminal = std::env::var_os("WT_SESSION").is_some()
            || std::env::var("TERM_PROGRAM").is_ok_and(|t| t == "WezTerm");
        if graphical_terminal {
            return Picker::from_query_stdio().unwrap_or_else(|_| Picker::from_fontsize((8, 16)));
        }
...
/// Whether the terminal's background is dark, deciding which base color the
/// UI's filled surfaces (hero panel, zebra rows, popups) are built from. On
/// a light-themed terminal a near-black fill reads as a hard black rectangle
/// pasted onto the page rather than a subtle panel.
///
/// Windows consoles report their actual background color through the console
/// API, so unlike the graphics-protocol query in `detect_picker` above there
/// is no escape sequence to write and no reply to wait for — nothing here can
/// stall startup. `wAttributes`' high nibble indexes `ColorTable`, giving the
/// real RGB of whatever scheme the user runs.
///
/// Defaults to dark whenever the answer isn't available (non-Windows, a
/// redirected/non-console stdout, or a failed call): dark is both the far
/// more common terminal default and what every color in this UI was
/// originally picked against.
///
/// Known limitation: verified working against conhost, where it correctly
/// reports the real scheme (Campbell's #0C0C0C reads back as luma 12). Under
/// Windows Terminal the console layer is ConPTY, which may hand back the
/// legacy default table rather than the profile's actual colors — so a WT
/// user on a light scheme can still land on the dark default. That's the
/// safe direction to be wrong in (it's today's behavior), but it means this
/// is a genuine improvement for conhost rather than a universal fix.
fn detect_dark_background() -> bool {
    #[cfg(windows)]
    {
        ...
        // SAFETY: `info` is zeroed and its required `cbSize` set before the
        // call, which is the documented contract; the handle is only read.
        // A non-console stdout (piped/redirected) makes the call fail rather
        // than misbehave, and that failure lands on the dark default.
        unsafe {
            let handle = GetStdHandle(STD_OUTPUT_HANDLE);
            let mut info: CONSOLE_SCREEN_BUFFER_INFOEX = std::mem::zeroed();
            info.cbSize = std::mem::size_of::<CONSOLE_SCREEN_BUFFER_INFOEX>() as u32;
            if GetConsoleScreenBufferInfoEx(handle, &mut info) == 0 {
                return true;
            }
            // Background is the high nibble of the attribute word; the
            // table stores colors as COLORREF (0x00BBGGRR).
            let bg_index = ((info.wAttributes >> 4) & 0x0F) as usize;
            let colorref = info.ColorTable[bg_index];
            let r = (colorref & 0xFF) as f32;
            let g = ((colorref >> 8) & 0xFF) as f32;
            let b = ((colorref >> 16) & 0xFF) as f32;
            // Rec. 601 luma — cheap, and precise enough for a binary choice.
...
/// Which of the two visualizer effects is showing — see `App::cycle_visualizer`.
/// Kept as two separate modes rather than one combined effect: rain and
/// particles read as distinct "looks," and showing both together made
/// neither one clearly legible.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl VisualizerMode {
    pub fn label(self) -> &'static str {
        match self {
            VisualizerMode::Rain => "Rain",
            VisualizerMode::Particles => "Particles",
        }
    }
}

/// State for the visualizer's digital-rain + rising-particle animation (see
/// `ui::visualizer`). Positions are in fractional terminal rows/columns —
/// `f32` rather than the eventual `u16` cell coordinates — so movement
/// speed isn't limited to whole cells per frame at a ~30fps redraw rate.
#[derive(Default)]
pub struct VisualizerFx {
    /// One entry per terminal column currently in view: the row position of
    /// that column's rain "head" (the brightest, leading character of its
    /// trail). Negative/beyond-height values mean the head hasn't entered
    /// (or has fully left) the visible area yet — used to stagger restarts.
    pub(crate) rain: Vec<f32>,
    pub(crate) particles: Vec<VizParticle>,
    /// (width, height) this state was last advanced for — a terminal
    /// resize invalidates `rain` (wrong column count) and gets it
    /// reinitialized rather than trying to remap it.
    pub(crate) size: (u16, u16),
}

pub(crate) struct VizParticle {
    pub(crate) x: f32,
    pub(crate) y: f32,
    pub(crate) vy: f32,
    /// 1.0 (just spawned) fading down to 0.0 (removed).
    pub(crate) life: f32,
    /// 0.0-1.0 position along the warm(bass)-to-cool(treble) color sweep,
    /// picked at spawn time so a particle keeps one color for its lifetime.
    pub(crate) hue_t: f32,
}
...
impl App {
    pub async fn new() -> Result<Self> {
        ...
        let mut app = Self {
            ...
            #[cfg(windows)]
            audio_capture: None,
            visualizer_fx: std::cell::RefCell::new(VisualizerFx::default()),
            playlists: Vec::new(),
            selected_playlist: 0,
            ...
            is_dark_bg: detect_dark_background(),
            artwork: None,
    ...
    /// Cycles Off -> Rain -> Particles -> Off. Starts/stops the WASAPI
    /// loopback capture thread on the Off<->on transitions only — switching
    /// between the two modes keeps it running, since both read the same
    /// `visualizer_bands`. mpv runs as a separate process we control over
    /// IPC and never see raw PCM from, so this captures system audio output
    /// independently instead (see `audio::capture`). Windows-only: the
    /// capture mechanism doesn't exist elsewhere.
    #[cfg(windows)]
    pub fn cycle_visualizer(&mut self) {
        let was_off = self.visualizer_mode.is_none();
        self.visualizer_mode = match self.visualizer_mode {
            None => Some(VisualizerMode::Rain),
            Some(VisualizerMode::Rain) => Some(VisualizerMode::Particles),
            Some(VisualizerMode::Particles) => None,
        };
            ...
            Some(mode) => {
                if was_off {
                    ...
                    if self.audio_capture.is_none() {
                        self.visualizer_mode = None;
                        self.status_message = "Visualizer: couldn't start audio capture.".to_string();
                        return;
                    }
                }
                self.status_message = format!("Visualizer: {}", mode.label());
            }
            None => {
    ...
    /// tick while the visualizer is on. No-op (cheap: one check) the rest
    /// of the time, and a no-op entirely off Windows.
    #[cfg(windows)]
    pub fn poll_visualizer(&mut self) {
        if self.visualizer_mode.is_none() {
            return;
        }
