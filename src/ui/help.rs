...
const BINDINGS: &[(&str, &str)] = &[
    ...
    ("v", "Cycle the live visualizer: off -> Rain -> Particles -> off (Now Playing tab, renders behind the artwork/info card)"),
...
pub fn draw(frame: &mut Frame, is_dark: bool) {
    let area = frame.area();
    ...
    // Opaque overlay over live content, same as the input prompt — it flips
    // fully rather than stepping subtly off the terminal background.
    let (overlay_bg, text_fg) = if is_dark {
    ...
    let mut lines: Vec<Line> = BINDINGS
        ...
        .map(|(key, action)| {
            Line::from(vec![
                ...
                Span::styled(format!("  {}", action), Style::default().fg(text_fg)),
            ])
        })
        .collect();
