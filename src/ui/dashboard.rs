...
fn draw_soundcloud(frame: &mut Frame, app: &App, area: Rect) {
    ...
    let selector: Vec<Span> = if app.soundcloud_username.is_empty() {
        ...
        vec![
            ...
            Span::styled(
                genre,
                crate::ui::theme::accent_bold(crate::ui::SELECTION),
            ),
            Span::styled(" ▸", muted_style()),
        ]
    } else {
        ...
        for (i, label) in labels.iter().enumerate() {
            // Selection is the one thing that must not ride the cover-art
            // accent: that colour changes with every track, so the highlight
            // would drift and sometimes land near the muted grey of the
            // labels it's supposed to stand out from.
