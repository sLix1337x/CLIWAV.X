...
- [x] Curated accent palette presets (`t`): Auto (artwork-driven) → Teal → Magenta → Amber → Violet, as an override for monochrome/washed-out covers that extract to a muddy accent (from UX research notes). Shown in the status bar; not persisted across restarts.
...
- [x] 10-band EQ (`7`, or Tab): driven through mpv's own `equalizer` libavfilter (10 chained instances, `31 Hz`-`16 kHz`), applied via a full `af set` replace rather than incremental `af-command` deltas — the latter was tested against a live mpv v0.41 and found unreliable even for well-documented runtime-commandable filters. Band/preset edits are debounced (200ms to mpv, 800ms to `config.toml`) since each replace can trigger a small audible blip. 8 built-in presets; gains persist and re-apply after a mid-session mpv restart. See `RESEARCH_NOTES.md` for the full investigation.
- [x] Live spectrum visualizer (`v`, Now Playing tab): since mpv is a separate process we control over IPC — no raw PCM access — this captures system audio output independently via WASAPI loopback (`audio::capture`, Windows-only, background thread + ring buffer) and runs its own 2048-sample FFT (`audio::spectrum`, `rustfft`) into 24 log-spaced bars with fast-attack/slow-decay smoothing and a silence gate. Off by default (real background CPU cost); toggling on switches the render tick to ~30fps (game_mode still overrides to the slower rate). Verified end-to-end with a throwaway `visualizer_check` bin (deleted after use, per the established `autoplay_check` pattern): real playing audio produced correct, moving, nonzero band data.
...
- [x] **Genre-seeded SoundCloud browse** (UX research notes): with no username configured, the category list/selector shows genre buckets (Trending, Hip-Hop, Electronic, House, Lo-Fi, Indie, Pop) backed by live `scsearch` — loads into the shared tracks pane with full list autoplay, on both the SoundCloud tab and the Dashboard (`←`/`→`).
